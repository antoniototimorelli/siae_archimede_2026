...
**Perché rischio MEDIUM:** la skill scrive o modifica file sorgente — componenti Vue, store Pinia, servizi Express. Un errore di tipo o di logica può rompere la build o introdurre regressioni. Il danno è reversibile via `git revert` ma richiede tempo di diagnosi.

**Come si attiva:** TRIGGER quando Claude sta per scrivere o modificare codice sorgente (componenti Vue, composable, store Pinia, route, servizi Express, file TypeScript). NON-TRIGGER per analisi e brainstorming prima della conferma utente, spiegazioni teoriche, modifiche a soli file di configurazione non-codice.

**Cosa garantisce il completamento:** il flusso obbligatorio impone che esista un test RED prima di aprire qualsiasi file di produzione e che lo step 4 invochi la skill `verification` — Claude non può dichiarare l'implementazione completa senza quel gate finale.
...
**Perché rischio MEDIUM:** la skill pilota la scrittura di ogni nuova unità di codice attraverso tre fasi sequenziali obbligatorie (RED → GREEN → REFACTOR). Saltare o invertire una fase può produrre test che non verificano nulla di reale o codice di produzione non coperto da alcun test.

**Come si attiva:** TRIGGER quando Claude sta per implementare una nuova funzione, metodo, componente Vue o modulo TypeScript (composable, store Pinia, service Express, utility). NON-TRIGGER per modifiche a test già esistenti senza nuova logica, refactor puro già coperto da test verdi, correzioni di typo o commenti.

**Cosa garantisce il completamento:** tre gate espliciti — output del test fallente (RED), output verde (GREEN), output finale con codice refactored (REFACTOR) — impediscono a Claude di avanzare senza prova concreta di ogni fase. Al termine del ciclo, la skill `verification` è obbligatoriamente invocata prima del commit.
...
**Perché rischio HIGH:** la skill è il gate finale prima di dichiarare un task completato. Un verdetto PASS errato — test non eseguiti, working tree sporco, errori di tipo ignorati — lascia passare codice difettoso in produzione. Il danno può essere difficile da rilevare a posteriori.

**Come si attiva:** TRIGGER quando chiunque pronuncia frasi di chiusura: `"ho finito"`, `"è fatto"`, `"posso fare il merge"`, `"task completato"`. NON-TRIGGER a metà implementazione, durante brainstorming o mentre si scrive un test.

**Cosa garantisce il completamento:** cinque step sequenziali con prove concrete — suite di test verde al 100%, `git status` pulito, conformità alle regole della skill `implementation`, type-check e linter senza errori. Un solo punto rosso blocca il PASS e rimanda allo step fallito.
